...
export default function GroupSpace() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // 2. Fetch Chat History (The fix for disappearing messages)
  ...
  const handleAddMentee = async (e) => {
    ...
    if (res.ok && data.success) {
      setMenteeEmail("");
      window.location.reload(); // Refresh to update roster
    } else {
  ...
  const handleAddResource = async (e) => {
    ...
    if (res.ok) {
      setResourceTitle("");
      setResourceUrl("");
      window.location.reload(); // Refresh to update resources
  ...
  const handleSendMessage = (e) => {
    ...
    if (currentMessage.trim() !== "") {
      socket.emit("send-group-message", {
        ...
        senderName: userName, // Using actual name from localStorage
      });
      setCurrentMessage(""); 
    }
  };
  ...
  const handleJoinVideoCall = () => {
    const roomName = `Mentorship-Platform-${groupId}`;
  ...
  return (
    <div style={{ padding: "40px", maxWidth: "1200px", margin: "0 auto", fontFamily: "Arial, sans-serif" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        ...
        <button onClick={handleJoinVideoCall} style={{ padding: "10px 20px", background: "#d9534f", color: "white", border: "none", borderRadius: "5px", cursor: "pointer", fontWeight: "bold" }}>
      ...
      <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
          ...
          <div style={{ background: "#f8f9fa", padding: "20px", borderRadius: "8px", border: "1px solid #ddd" }}>
            ...
            {isMentor && (
              <form onSubmit={handleAddMentee} style={{ display: "flex", gap: "10px", marginBottom: "15px" }}>
                ...
                <button type="submit" style={{ padding: "8px 15px", background: "#007bff", color: "white", border: "none" }}>Add</button>
              </form>
            )}
          ...
          <div style={{ background: "#f8f9fa", padding: "20px", borderRadius: "8px", border: "1px solid #ddd" }}>
            ...
            {isMentor && (
              <form onSubmit={handleAddResource} style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "15px" }}>
                ...
                <button type="submit" style={{ padding: "8px 15px", background: "#28a745", color: "white", border: "none" }}>Upload</button>
              </form>
            )}
            <ul style={{ paddingLeft: "20px", margin: 0 }}>
        ...
        <div style={{ flex: 2, minWidth: "400px", background: "#fff", border: "1px solid #ddd", borderRadius: "8px", display: "flex", flexDirection: "column", height: "600px" }}>
          ...
          <div style={{ flex: 1, padding: "20px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "12px", backgroundColor: "#f4f4f9" }}>
            {messages.length === 0 ? (
              <p style={{ textAlign: "center", color: "#888", marginTop: "auto", marginBottom: "auto" }}>No messages yet. Say hello!</p>
            ) : (
              messages.map((msg, index) => (
                <div key={index} style={{
                  ...
                  <div style={{ fontSize: "10px", textAlign: "right", marginTop: "4px", opacity: 0.5 }}>
                    {msg.time}
                  </div>
                </div>
              ))
